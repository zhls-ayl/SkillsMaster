...
- 若本次任务预计需要推送代码，先确认仓库默认分支是否受 GitHub branch protection 约束；若必须通过 PR 合并，应一开始就从 `codex/<topic>` 之类的工作分支开展，而不是在 `main` 上完成后再补切分支

## 开发流程
1. 先确认需求、范围、风险点与交付物
2. 阅读相关 `Service` / `ViewModel` / `View` / `Tests`
3. 判断是否涉及路径、迁移、lock file、仓库同步、Release 脚本等高风险区域
4. 若页面存在多个入口或展示模式，先列出这些入口，再做实现；不要默认“默认路径能用”就等于全部路径正确
5. 做最小可验证变更（minimal change）
6. 补充或更新相邻测试用例
7. 更新对应文档
8. 最后检查 README、docs 与代码命名是否一致
...
- 代码改动默认要补测试，尤其是：
  ...
  - ViewModel 的状态转换
- 对复用型详情页或共享子视图，除了主路径测试外，还应补一轮“次级入口 / 只读模式 / 权限裁剪”检查，避免按钮、编辑能力或系统跳转在非默认入口下意外泄漏
